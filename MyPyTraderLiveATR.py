...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                             ...
                             QCheckBox, QTableWidget, QTableWidgetItem, QAction, QHeaderView, QAbstractItemView, QDateTimeEdit)
from PyQt5.QtGui import QColor, QPalette
from PyQt5.QtCore import Qt, QSize, QTimer, QThread, pyqtSignal, QDateTime
...
class ArchiveWorker(QThread):
    ...
    def create_or_continue_file(self, file_path, start_time):
        ...
        self.live.subscribe(
            ...
            schema="ohlcv-1m",
            stype_in="continuous",
            symbols=["MES.c.0", "MNQ.c.0", "MCL.c.0", "MGC.c.0, ES.c.0", "NQ.c.0", "CL.c.0", "GC.c.0"],
            start=start_time
        )
...
class TradingApp(QMainWindow):
    def __init__(self):
        ...
        self.setup_ui()
        self.on_stop_loss_type_changed(self.stop_loss_type_combo.currentText())
        ...
        #Enable break even by default
        self.be_checkbox.setChecked(True)
        #self.move_stop_loss_to_break_even(True)
    ...
    def setup_ui(self):
        ...
        self.stop_loss_type_combo.currentTextChanged.connect(self.on_stop_loss_type_changed)
        stop_loss_layout.addWidget(self.stop_loss_type_combo, 1, 1)


        # Add the BE checkbox
        self.be_checkbox = QCheckBox("Break Even")
        self.be_checkbox.stateChanged.connect(self.on_be_checkbox_changed)
    ...
    def on_stop_loss_type_changed(self, stop_type):
        if stop_type == "Trailing":
            #self.stop_loss_input.setReadOnly(True)
            self.stop_loss_input.setEnabled(False)
            #self.stop_loss_input.setStyleSheet("background-color: #f0f0f0;")  # Gray out the field
            #self.trail_by_input.setReadOnly(False)
            #self.trail_by_input.setStyleSheet("")  # Reset style
        else:
            self.stop_loss_input.setEnabled(True)
            #self.stop_loss_input.setStyleSheet("")  # Reset style
    ...
    def add_or_update_trade(self):
        ...
        if dialog.exec_():
            ...
            # Enable or disable break-even checkbox based on TP levels
            self.be_checkbox.setEnabled(bool(self.tp_levels[current_ticker]))
            ...
            try:
                ...
                if response_data.get("success"):
                    ...
                    if remaining_quantity <= 0:
                        ...
                        # Update trailing stop only if it's a trailing stop type
                        stop_loss = self.active_orders[ticker].get('stop_loss', {})
                        if stop_loss.get('type') in ['trailing_stop', 'trail_after_1st_tp']:
    ...
    def update_trailing_stop(self, ticker, current_price, remaining_quantity):
        order = self.active_orders[ticker]
        stop_loss = order['stop_loss']
        trail_amount = stop_loss.get('trailAmount')
        
        if not trail_amount:
            print(f"Warning: No trail amount found for {ticker}")
            return
        
        # Get the current stop price, or calculate initial stop if none exists
        current_stop_price = stop_loss.get('stopPrice')
        if current_stop_price is None:
            # Set initial stop price based on action
            if order['action'] == 'buy':
                current_stop_price = current_price - trail_amount
            else:  # sell
                current_stop_price = current_price + trail_amount
        
        # Calculate new stop price
        if order['action'] == 'buy':
            new_stop_price = max(current_price - trail_amount, current_stop_price)
        else:  # sell
            new_stop_price = min(current_price + trail_amount, current_stop_price)

        # Update stop loss if it changed
        if new_stop_price != current_stop_price:
            stop_loss['stopPrice'] = new_stop_price
            self.update_stop_loss_display(ticker)
            self.save_active_orders()
            self.update_broker_stop_loss(ticker, new_stop_price, "trailing_stop")
            print(f"Updated trailing stop for {ticker} to {new_stop_price}")

        # Add debug logging
        print(f"Trailing stop update for {ticker}:")
        print(f"Current price: {current_price}")
        print(f"Trail amount: {trail_amount}")
        print(f"Current stop: {current_stop_price}")
        print(f"New stop: {new_stop_price}")


    def activate_trailing_stop(self, ticker, current_price):
        order = self.active_orders[ticker]
        stop_loss = order['stop_loss']
        trail_amount = stop_loss['trailAmount']

        if order['action'] == 'buy':
            initial_stop_price = current_price - trail_amount
        else:  # sell
            initial_stop_price = current_price + trail_amount
        ...
        update_order = {
            ...
            "signalPrice": str(int(current_price)),
        ...
        try:
            ...
            if response_data.get("success"):
                new_stop_loss = {
                    ...
                    "lastStopPrice": initial_stop_price
                }
                order['stop_loss'] = new_stop_loss
                self.update_stop_loss_display(ticker)
                self.save_active_orders()
                self.update_response_area(f"Activated trailing stop for {ticker}. Initial stop price: {initial_stop_price:.2f}, Trail amount: {trail_amount}\n")
            else:
                self.update_response_area(f"Error activating trailing stop: {response_data}\n")
        except requests.RequestException as e:
            self.update_response_area(f"Error activating trailing stop: {str(e)}\n")



    def check_and_update_stop_loss(self, ticker, current_price):
        if ticker not in self.active_orders:
            print(f"No active order for {ticker}")
            return
        ...
        print(f"Checking stop loss for {ticker}: Action: {action}, Current Price: {current_price}, Stop Price: {stop_price}, Trail Amount: {trail_amount}")

        # Check initial stop loss
        if stop_loss['type'] in ['stop', 'stop_limit', 'break_even', 'trail_after_1st_tp']:
            if (action == 'buy' and current_price <= stop_price) or \
            (action == 'sell' and current_price >= stop_price):
                print(f"Initial stop loss hit for {ticker}: Current Price: {current_price}, Stop Price: {stop_price}")
                self.execute_stop_loss(ticker, current_price)
                return

        # Break-even logic
        if stop_loss['type'] == 'break_even':
            if (action == 'buy' and current_price >= stop_price + trail_amount) or \
            (action == 'sell' and current_price <= stop_price - trail_amount):
                print(f"Activating trailing stop for {ticker}")
                self.activate_trailing_stop(ticker, current_price)

        # Trailing stop logic
        elif stop_loss['type'] == 'trailing_stop':
            last_stop_price = stop_loss.get('lastStopPrice', stop_price)
            if action == 'buy':
                new_stop_price = max(current_price - trail_amount, last_stop_price)
                if current_price <= new_stop_price:
                    print(f"Trailing stop hit for {ticker}: Current Price: {current_price}, New Stop Price: {new_stop_price}")
                    self.execute_stop_loss(ticker, current_price)
                    return
                elif new_stop_price > last_stop_price:
                    stop_loss['lastStopPrice'] = new_stop_price
                    self.save_active_orders()
                    print(f"Updated trailing stop for {ticker}: New Stop Price: {new_stop_price}")
            else:  # sell
                new_stop_price = min(current_price + trail_amount, last_stop_price)
                if current_price >= new_stop_price:
                    print(f"Trailing stop hit for {ticker}: Current Price: {current_price}, New Stop Price: {new_stop_price}")
                    self.execute_stop_loss(ticker, current_price)
                    return
    ...
    def execute_stop_loss(self, ticker, current_price):
        print(f"=== Executing stop loss for {ticker} at price {current_price} ===")
        exit_order = {
            "ticker": self.ticker_map.get(ticker, ticker),
            "action": "exit",
            "orderType": "market",
            "quantity": str(self.active_orders[ticker]['quantity'])
        }
        print(f"Prepared exit order: {exit_order}")
        try:
            print("Sending exit order to server...")
            response = requests.post(self.api_url, json=exit_order)
            response.raise_for_status()
            ...
            if response_data.get("success"):
                print(f"Stop loss successfully executed for {ticker}")
                self.update_response_area(f"Stop loss hit for {ticker} at price {current_price:.2f}. Exit order sent.\n")
                print(f"Clearing trade for {ticker}")
                self.clear_trade(ticker)
                self.update_trade_status()
                self.update_tp_table()
            else:
                print(f"Error: Server reported failure in executing stop loss for {ticker}")
                self.update_response_area(f"Error sending exit order for stop loss: {response_data}\n")
        except requests.RequestException as e:
            print(f"Exception occurred while sending exit order: {str(e)}")
            self.update_response_area(f"Error sending exit order for stop loss: {str(e)}\n")
    ...
    def move_stop_loss_to_break_even(self, ticker):
        ...
        tick_size = self.tick_sizes.get(ticker, 0.01)
        trail_amount = float(self.trail_by_input.text())

        if order['action'] == 'buy':
            new_stop_price = entry_price + tick_size
        else:  # sell
            new_stop_price = entry_price - tick_size
        ...
        self.update_stop_loss_display(ticker)
        self.save_active_orders()
        self.update_response_area(f"Stop loss moved to break even for {ticker} at {new_stop_price:.2f}\n")

        # Update the broker's stop loss
        self.update_broker_stop_loss(ticker, new_stop_price, "stop")


    def update_broker_stop_loss(self, ticker, stop_price, stop_type):
        ...
        update_order = {
            ...
            "action": "exit",
            "orderType": stop_type,
            "stopPrice": str(int(stop_price)),
            "quantity": str(order['quantity'])
        }

        if stop_type == "trailing_stop":
            update_order["trailAmount"] = str(order['stop_loss']['trailAmount'])
        ...
        try:
            ...
            response_data = response.json()
            if response_data.get("success"):
                self.update_response_area(f"Updated broker stop loss for {ticker}. Stop price: {stop_price}, Type: {stop_type}\n")
            else:
                self.update_response_area(f"Error updating broker stop loss: {response_data}\n")
        except requests.RequestException as e:
            self.update_response_area(f"Error updating broker stop loss: {str(e)}\n")
    ...
    def handle_databento_data(self, subscription_id, message):
        try:
            ...
            if isinstance(message, db.TradeMsg):
                ...
                if ticker:
                    price = message.price / 1e9  # Convert nanoseconds to standard price
                    side = message.side
                    size = message.size
                    ts_event = message.ts_event

                    self.update_price_display(ticker, price, side)
                    
                    if ticker == self.ticker_combo.currentText():
                        # Check and update stop loss
                        if ticker in self.active_orders:
                            print(f"Checking stop loss for {ticker} at price {price}")
    ...
    def update_price_display(self, ticker, price, side):
        self.current_prices[ticker] = price
        
        if ticker == self.ticker_combo.currentText():
            if side == 'A':  # Ask side (buy)
                color = "green"
            elif side == 'B':  # Bid side (sell)
                color = "red"
            else:  # 'N' or any other value
                color = "black"
            
            self.price_input.setStyleSheet(f"color: {color};")
            self.price_input.setText(f"{price:.2f}")
    ...
    def update_stop_loss_display(self, ticker):
        if ticker in self.active_orders:
            order = self.active_orders[ticker]
            stop_loss = order.get('stop_loss')
            ...
            if stop_loss:
                stop_type = stop_loss['type']
                if stop_type == 'trailing_stop':
                    # Convert last_stop_price to float, default to current_price if N/A
                    last_stop_price = stop_loss.get('lastStopPrice', current_price)
                    if isinstance(last_stop_price, str) and last_stop_price != 'N/A':
                        last_stop_price = float(last_stop_price)
                    elif last_stop_price == 'N/A':
                        last_stop_price = current_price
                    
                    trail_amount = float(stop_loss.get('trailAmount', 0))
                    
                    if order['action'] == 'buy':
                        current_stop = max(current_price - trail_amount, last_stop_price)
                    else:  # sell
                        current_stop = min(current_price + trail_amount, last_stop_price)
                    
                    # Format the display string with proper floating point numbers
                    self.stop_loss_price_label.setText(
                        f"SL @: {current_stop:.2f} (Trailing, Last: {last_stop_price:.2f})"
                    )
                
                elif stop_type == 'break_even':
                    stop_price = float(stop_loss.get('stopPrice', 0))
                    self.stop_loss_price_label.setText(f"SL @: {stop_price:.2f} (Break Even)")
    ...
    def on_be_checkbox_changed(self, state):
        current_ticker = self.ticker_combo.currentText()
        # if current_ticker in self.active_orders:
        if state == Qt.Checked:
            if current_ticker in self.tp_levels and any(tp['enabled'] for tp in self.tp_levels[current_ticker]):
                self.update_response_area("Break-even activated. Will move stop loss to entry price after first TP hit.\n")
            else:
                self.be_checkbox.setChecked(False)
                self.update_response_area("Cannot activate break-even without any active TP levels.\n")
        else:
            self.update_response_area("Break-even deactivated.\n")
        # else:
        #     self.be_checkbox.setChecked(False)
        #     self.update_response_area("Cannot activate break-even without an active trade.\n")
    ...
    def send_order(self, action):
        ...
        if action in ["buy", "sell"]:
            ...
            # Initialize stop_loss_price as None
            stop_loss_price = None
            
            if stop_loss_type == "Trailing":
                trail_amount = float(self.trail_by_input.text())  # Use trail by amount instead
                broker_stop_loss = {
                    "type": "trailing_stop",
                    "trailAmount": trail_amount
                }
            ...
            # Set local stop loss info if not already set
            if not local_stop_loss_info:
                local_stop_loss_info = {
                    "type": self.get_stop_loss_type(stop_loss_type),
                    "stopPrice": stop_loss_price,
                    "trailAmount": float(self.trail_by_input.text())
    ...
    def initialize_databento_worker(self):
        ...
        try:
            ...
            self.update_response_area(f"Databento worker initialized for {current_ticker}. Starting to receive trade updates.\n")
        except Exception as e:
